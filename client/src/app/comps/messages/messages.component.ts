    ...
    next: (messages) => {
      ...
      forkJoin(requests).subscribe(users => {
        this.chats = users
          ...
          .map(u => {
            const msgs = messages.filter(m => m.senderId === u.userId || m.receiverId === u.userId);
            const lastMsg = msgs.sort((a,b) => new Date(b.sentAt).getTime() - new Date(a.sentAt).getTime())[0];
